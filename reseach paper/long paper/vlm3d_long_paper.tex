...
% Author block (toggle between camera-ready and double-blind submission)
...
% For double-blind MICCAI review phase, uncomment below and comment above:
...
\item \textit{Pervasive label co-occurrence and imbalance}: Positive class prevalence ranges from 7.4\% (pericardial effusion) to 44.8\% (lung nodule; lung opacity at 39.0\%), producing acute gradient starvation for low-prevalence findings when trained under standard binary cross-entropy.
\end{enumerate}
...
\noindent\textbf{Resampling and Augmentation.} Volumetric scans are resampled to isotropic $1.5$~mm spacing via trilinear interpolation, cropped to foreground tissue, and zero-padded to a fixed input tensor of $160 \times 160 \times 112$ voxels. Training data are augmented with random affine transformations (rotations $\pm 10^\circ$, scaling $0.9-1.1$), intensity shifts, and spatial axis flips.
...
Under severe multi-label imbalance, standard Binary Cross-Entropy (BCE) gradients are heavily dominated by easy negative background instances. We train all models using Asymmetric Loss (ASL)~\cite{ridnik2021}, which builds upon focal loss formulations~\cite{lin2017} and label smoothing regularisation~\cite{szegedy2016} by decoupling positive and negative focusing:
\begin{equation}
...
\end{equation}
Averaging predictions across all 8 spatial variants reduces aleatoric uncertainty and orientation bias~\cite{wang2019}.
...
Only two pathologies remain below AUROC 0.70: Lung nodule (0.663) and Pulmonary fibrotic sequelae (0.651). At 1.5 mm isotropic spacing, sub-centimetre pulmonary nodules span only 3--5 voxels, losing distinctive boundary details. Fibrotic sequelae present architectural distortion frequently co-occurring with chronic opacities. Conversely, Pleural effusion (0.928) and Cardiomegaly (0.896) achieve superior ranking accuracy owing to distinct fluid-attenuation boundaries and organ-silhouette geometry easily detected by volumetric attention.
...
\begin{enumerate}
\setlength{\itemsep}{1pt}\setlength{\parskip}{0pt}
\item \textit{Spatial resolution limit}: Downsampling volumes to $1.5$~mm isotropic spacing degrades fine interstitial and micro-nodular patterns. Future work will explore 2-stage patch-based architectures or gradient checkpointing to support native $1.0$~mm resolution.
